...
class GuessWhoGame
  ...
  def play
    ...
    if @people_left.count == 1
      ...
      if @num_guesses >= NUM_CHANCES
        ...
        puts "You should have guessed: #{@person}"
        return false
      end
    end
    return true
  ...
  def ask_for_guess
    ...
    key = gets.chomp
    return key, "" if key == "gender" || key == "skin"
  ...
  def filter_people_left(key, value)
    if key == "gender"
      @people_left.select!{ |p| p.has_attribute?("gender", @person.attributes["gender"]) }
    elsif key == "skin"
      @people_left.select!{ |p| p.has_attribute?("skin", @person.attributes["skin"]) }
    else
